      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {alumniData.map((alumni) => (
            ...
            >
              {/* Photo Avatar */}
              <div className="w-24 h-24 mb-4 rounded-full overflow-hidden border-4 border-slate-50 group-hover:border-primary/10 transition-colors">
                <img 
                  src={alumni.image} 
                  alt={alumni.name} 
                  className="w-full h-full object-cover"
                  onError={(e) => {
                    // Fallback avatar incase image breaks
      ...
      <div className="bg-slate-50 rounded-2xl p-8 lg:p-12 border border-slate-100 mt-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {testimonials.map((testim) => (
            <div key={testim.id} className="bg-white p-8 rounded-xl shadow-sm border border-slate-100 relative group hover:shadow-soft transition-all duration-300">
