...
import { fetchAlumni, deleteAlumni } from '../../../api/admin/alumniService';
...
const AlumniList = () => {
  ...
  const getImageUrl = (path) => {
    if (!path) return 'https://via.placeholder.com/150?text=Alumni';
  ...
  const loadAlumni = async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to fetch alumni members');
      toast.error(err.message || 'Failed to fetch alumni members');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="p-4">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1">Manage and connect with department alumni</p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="bg-red-50 text-red-700 p-4 rounded-lg border border-red-100 flex items-center gap-3">
          ...
          {error}
          <button onClick={loadAlumni} className="ml-auto underline font-medium">Retry</button>
        </div>
      ) : (
                ...
                {alumni.length === 0 ? (
                  <tr>
                    <td colSpan="5" className="p-10 text-center text-gray-400">
                      No alumni records found. Click "Add Alumni" to create one.
                    </td>
                  </tr>
                ) : (
                  alumni.map(person => (
                    <tr key={person._id} className="border-b last:border-0 border-gray-50 hover:bg-gray-50/50 transition-colors">
                      <td className="p-4">
                        <img 
                          src={getImageUrl(person.image)} 
                          alt={`Profile of ${person.name}`}
                          className="h-12 w-12 rounded-full object-cover border-2 border-indigo-50 shadow-sm transition-transform hover:scale-110"
                        />
                      </td>
                      <td className="p-4">
                      ...
                      <td className="p-4 text-gray-600 font-medium">{person.company || 'N/A'}</td>
                      <td className="p-4 text-right">
                        <div className="flex justify-end gap-3">
                          <button
                            onClick={() => navigate(`edit/${person._id}`)}
